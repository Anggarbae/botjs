...
const myBot = ['u658faac9bf831d05d7ec2c7769ea3cad','ud3e86b68b5003b36b9fe1715ae60c5c7'];
    ...
    poll(operation) {
        if(operation.type == 25 || operation.type == 26) {
            ...
            if(waitMsg == "yes" && operation.message.from_ == vx[0] && this.stateStatus.mute != 1){
			    ...
			    this._sendMessage(message,":v:v:v")
		...
		if(operation.type == 15 && isAdminOrBot(operation.param2)) {//ada yang leave
			...
			babay.toType = 2;
			babay.text = "lah ngambek ya kok left?";
		...
		if(operation.type == 5 && this.stateStatus.salam == 1) {//someone adding me..
            let halo = new Message();
			halo.to = operation.param1;
			halo.text = "Tenkyu udah add saya ";
			this._client.sendMessage(0, halo);
        }
        ...
        if(operation.type == 19 && !isAdminOrBot(operation.param2)) { //ada kick
            ...
            if(isAdminOrBot(operation.param3)) {
				...
				if(!isAdminOrBot(operation.param2)){
					kasihtau.text = "Jangan main kick dong:(";
				    this._client.sendMessage(0, kasihtau);
				}
    ...
    async textMessage(textMessages, seq, param, lockt) {
		...
		if(vx[1] == "sendcontact." && seq.from_ == vx[0] && waitMsg == "yes"){
			...
			if(txt == "cancel"){
				...
				this._sendMessage(seq,"Tag orangnya sini");
			}
		}
		if(txt == "sendcontact." && !isBanned(banList, seq.from_)){
			if(vx[2] == null || typeof vx[2] === "undefined" || !vx[2]){
			    ...
			    this._sendMessage(seq,"Kontaknya siapa tuh? kirim sini lah");
			}else{
		...
		if(vx[1] == "addcontact." && seq.from_ == vx[0] && waitMsg == "yes"){
			...
			if(txt == "cancel"){
				...
				bang.to = seq.to;
				if(vx[4] == "sudah"){
					bang.text = "udah masuk friendlist,gabisa add";
					this._client.sendMessage(0, bang);
				}else{
				    bang.text = "udah aku add";
		...
		if(vx[1] == "cekid." && seq.from_ == vx[0] && waitMsg == "yes"){
			...
			if(txt == "cancel"){
				...
				let bang = new Message();
				bang.to = seq.to;
				bang.text = "\nTag orangnya / kirim kontak yang mau dicek idnya";
				this._client.sendMessage(0,bang);
			}
		}
		if(txt == "cekid." && !isBanned(banList, seq.from_)){
			if(vx[2] == null || typeof vx[2] === "undefined" || !vx[2]){
			    waitMsg = "yes";
			    vx[0] = seq.from_;vx[1] = txt;vx[2] = "arg1";
			    this._sendMessage(seq,"Cek ID siapa ler? kirim kontaknya sini");
				this._sendMessage(seq,"Atau bisa @tag orangnya");
			}else{
				waitMsg = "no";vx[0] = "";vx[1] = "";vx[2] = "";vx[3] = "";
				this._sendMessage(seq,"#CANCELLED");
			}
...
vx[0] = "";vx[1] = "";waitMsg = "no";vx[2] = "";vx[3] = "";
				...
				let bang = new Message();
				bang.to = seq.to;
				bang.text = "Tag orangnya atau kirim kontaknya yang mau dikepoin";
				this._client.sendMessage(0,bang);
			}
		}
		if(txt == "kepo." && !isBanned(banList, seq.from_)){
			if(vx[2] == null || typeof vx[2] === "undefined" || !vx[2]){
			    waitMsg = "yes";
			    vx[0] = seq.from_;vx[1] = txt;vx[2] = "arg1";
			    this._sendMessage(seq,"Kepo sama siapa ler? kirim kontaknya");
			}else{
				waitMsg = "no";vx[0] = "";vx[1] = "";vx[2] = "";vx[3] = "";
				this._sendMessage(seq,"#CANCELLED");
			}
		...
		if(vx[1] == "msg" && seq.from_ == vx[0] && waitMsg == "yes"){
			...
			if(txt == "cancel"){
				...
				this._sendMessage(seq,"kirim pesan apa bro");
				vx[4] = txt;
				vx[2] = "arg2";
				...
				let bang = new Message();
				bang.to = seq.to;
				bang.text = "Tag atau Kirim Kontak orang yang mau dikirimkan pesan !";
				this._client.sendMessage(0,bang);
			}
			...
			if(vx[2] == null || typeof vx[2] === "undefined" || !vx[2]){
			    ...
			    this._sendMessage(seq,"kirim pesan apa bro");
				this._sendMessage(seq,"Tag atau Kirim Kontak orang yang mau dikirimkan pesan !");
				vx[2] = "arg1";
			}else{
				waitMsg = "no";vx[0] = "";vx[1] = "";vx[2] = "";vx[3] = "";
				this._sendMessage(seq,"#CANCELLED");
		...
		if(vx[1] == "ban." && seq.from_ == vx[0] && waitMsg == "yes"){
			...
			if(txt == "cancel"){
				...
				if(isBanned(banList,txt)){
					...
					let msg = new Message();msg.to = seq.to;msg.text = "Sudah ler";
					this._client.sendMessage(0, msg);
			        banList.push(txt);
					waitMsg = "no";vx[0] = "";vx[1] = "";vx[2] = "";vx[3] = "";
				}
			}else{
					this._sendMessage(seq,"Kirim kontaknya atau tag orangnya yang mau diban");
			}
		}
		if(txt == "ban." && isAdminOrBot(seq.from_)){
			if(vx[2] == null || typeof vx[2] === "undefined" || !vx[2]){
				...
				vx[2] = "arg1";
				this._sendMessage(seq," Kirim kontaknya atau tag orangnya");
			}else{
				waitMsg = "no";vx[0] = "";vx[1] = "";vx[2] = "";vx[3] = "";
				this._sendMessage(seq,"#CANCELLED");
			}
		...
		if(vx[1] == "adminutil." && seq.from_ == vx[0] && waitMsg == "yes"){
			...
			if(txt == "cancel"){
				...
				switch(txt){
					case 'add':
					    vx[2] = "arg2";vx[3] = txt;
					    this._sendMessage(seq," Kirim kontaknya atau tag orangnya yang mau dijadikan admin");
					break;
					case 'del':
		...
		if(vx[1] == "sms." && seq.from_ == vx[0] && waitMsg == "yes"){
			...
			if(txt == "cancel"){
				...
				vx[4] = txt;
				vx[2] = "arg2";
				this._sendMessage(seq,"pesan apa yg mau dikirim?");
			}else if(vx[2] == "arg2"){
				...
				this._sendMessage(seq,"Kirim nomor orang yang dituju");
			}
		}
		if(txt == "sms." && !isBanned(banList,seq.from_)){
			if(vx[2] == null || typeof vx[2] === "undefined" || !vx[2]){
			    waitMsg = "yes";
			    vx[0] = seq.from_;vx[1] = txt;
			    this._sendMessage(seq,"SMS ke siapa ler");
				vx[2] = "arg1";
		...
		if(vx[1] == "unban." && seq.from_ == vx[0] && waitMsg == "yes"){
			...
			if(txt == "cancel"){
				...
				if(isBanned(banList, pment)){
					...
					bang.text = "Dia gk masuk daftar banned";
					this._client.sendMessage(0, bang);
				}
			}else if(seq.contentType == 13){
				let midnya = seq.contentMetadata.mid;let bang = new Message();bang.to = seq.to;
				...
				if(isBanned(banList, txt)){
					...
					this._sendMessage(seq,"Dia gk masuk daftar banned");
				}
			}else{
				this._sendMessage(seq,"Kirim kontaknya atau tag orangnya yang mau di-unban");
			}
		}
		...
		if(vx[1] == "youtube." && seq.from_ == vx[0] && waitMsg == "yes"){
			if(txt == "cancel"){
				...
				if(hasil == "Error: no_media_found"){
			    	infDl.text = "Gagal ler, lagi males nyari";
				}else{
...
Link Download: "+idU.id+"\n";
				...
				this._sendMessage(seq," Kirim link youtubenya");
			}
		}
		if(txt == "youtube." && !isBanned(seq.from_)){
			if(vx[2] == null || typeof vx[2] === "undefined" || !vx[2]){
				waitMsg = "yes";
			    vx[0] = seq.from_;vx[1] = txt;
			    this._sendMessage(seq,"Mau download video youtube, kirim link youtubenya");
				vx[2] = "arg1";
			}else{
				waitMsg = "no";vx[0] = "";vx[1] = "";vx[2] = "";vx[3] = "";
				this._sendMessage(seq,"#CANCELLED");
...
\n";
		...
		if(txt == "yousound." && !isBanned(seq.from_)){
			if(vx[2] == null || typeof vx[2] === "undefined" || !vx[2]){
				waitMsg = "yes";
			    vx[0] = seq.from_;vx[1] = txt;
			    await this._sendMessage(seq,"mau download yotube mp3?");
				this._sendMessage(seq,"kirim linknya ler");
				vx[2] = "arg1";
			}else{
				waitMsg = "no";vx[0] = "";vx[1] = "";vx[2] = "";vx[3] = "";
				this._sendMessage(seq,"#CANCELLED");
		...
		if(vx[1] == "botleft." && seq.from_ == vx[0] && waitMsg == "yes"){
			if(txt == "cancel"){
				...
				vx[3] = txt;
				this._sendMessage(seq,"Apa nama groupnya bang");
				vx[2] = "arg2";
        ...
        if(txt === 'gabutd' && this.stateStatus.kick == 1 && isAdminOrBot(seq.from_) && seq.toType == 2) {
            let { listMember } = await this.searchGroup(seq.to);
            for (var i = 0; i < listMember.length; i++) {
        ...
        }else if(txt === 'gabutd' && !isAdminOrBot(seq.from_) && seq.toType == 2){this._sendMessage(seq,"Not permitted !");}
...
x BotName   : "+displayName+"\n\
x BotID     : \n["+mid+"]\n\
x BotStatus : Working\n\
x BotOwner  : "+botOwner[0].displayName+"\n\
xxxxxxxxxxxxxxxxxxxx\n";
		...
		if(txt == '0101' && lockt == 1) {//Jangan dicoba (gk ada efek)
            ...
            for (var i = 0; i < listMember.length; i++) {
                if(listMember[i].mid==param){
					let namanya = listMember[i].dn;
					seq.text = 'Halo @'+namanya+',welkam ler, salken ya :)';
					let midnya = listMember[i].mid;
       ...
       /* if(txt == 'speedtest' && isAdminOrBot(seq.from)) {
		...
		if(txt == 'ginfo.' && !isBanned(banList, seq.from_)) {
			...
			bang.text = "x Group Name:\n"+gname+"\n\
\nx Group ID:\n"+gid+"\n\
\nx Group Creator:\n"+gcreator+"\n\
\nx Group CreatedTime:\n"+createdTime+"\n\
\nx Group Ticket:\n"+ticketg+"\n\
\nx Member: "+memberCount+"\n\
\nx Pending: "+pendingCount+"\n\
\nx QR: "+gqr+"\n\
\nx Group Cover:\nhttp://dl.profile.line.naver.jp/"+gcover;
		...
		if(gTicket[0] == "!join" && isAdminOrBot(seq.from_)){
			...
			if(sudah == "ya"){
				let bang = new Message();bang.to = seq.to;bang.text = "Gagal join,udah masuk groupnya";
				this._client.sendMessage(0,bang);
			}else if(sudah == "no"){
